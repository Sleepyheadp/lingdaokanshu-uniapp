...
<script>
  import {
    barChartCountryIndustry, multiBarChartHorizontal6, horizontalBarsruarl, horizontalBarsruarl2,
  ...
  export default {
    ...
    methods: {
      initWin() {
        let nameArray = [];
        this.Poverty.goalsAchieved.data.xaxis.forEach(item => {
          item = item.split('目标完成率')[0];
          nameArray.push(item);
        });
        this.Poverty.goalsAchieved.data.xaxis = nameArray;
        this.Poverty.goalsAchieved.data.gridLeft =  remTorpx(this.changeWithWidth(-273,-278,-270)/75);;
        setTimeout(() => {
          //人均收入
          horizontalBarsruarl2('Revitaliza',this.Poverty.goalsAchieved.data);
          // horizontalBars('Revitaliza',this.Poverty.goalsAchieved.data);
        }, 500);
      },
